...
public class Raiz {
    ...
    public void insertar (int persona,char departamento,char bebida) {
        if (primerNodo.tengoHijos==false) {
            int j = 0;
            for (int i = 0; i<primerNodo.Personas.length; i++) {
                if (primerNodo.Personas[i] == 0) {
                    primerNodo.Personas[i] = persona;
                   Lista.ingresados.add(persona);
 
                    j = i;
                    ordenar(primerNodo.Personas,6);
                    break;
                }
            }
            if (j == 2*grado) {
            ...
            ingresarEnHijos(primerNodo, persona,departamento,bebida);
    ...
    public void ingresarEnHijos(Nodo conHijos, int persona,char departamento,char bebida) {
        boolean entro = false;
        if(conHijos != null && !conHijos.tengoHijos){
            ubicarValorEnArreglo(conHijos, persona,departamento,bebida);
            entro = true;
        }
        for(int i = 0; conHijos != null && i < 2*grado + 1  && !entro; i++){
            if(persona < conHijos.Personas[i] || conHijos.Personas[i] == 0){
                entro = true;
                ingresarEnHijos(conHijos.nodo[i], persona,departamento,bebida);
                i = 2*grado;
            }
        } 
    }
    public void ubicarValorEnArreglo(Nodo nodoA, int persona,char departamento,char bebida){
        int cont = 0;
        while(cont <= 2*grado){
            if (nodoA.Personas[cont]==0) { 
                nodoA.Personas[cont]=persona;
                ordenar(nodoA.Personas, 5);
                Lista.ingresados.add(persona);
                if (cont == 2*grado) {
                    split(nodoA);
                }
     ...
     public void ordenarNodos(Nodo aOrdenar){
       ...
       while(i < 2 * grado + 3 && aOrdenar.nodo[i] != null){
           ...
           while(j < 2 * grado +2  && aOrdenar.nodo[j] != null && aOrdenar.nodo[j+1] != null){
               if(aOrdenar.nodo[j].Personas[0] > aOrdenar.nodo[j+1].Personas[0] ){
                   tmp = aOrdenar.nodo[j];
    ...
    public void split (Nodo nodo) {
        ...
        for (int i =0; i<grado; i++){ //guarda los valores en hijoIzq e hijoDer
            hijoIzq.Personas[i] = nodo.Personas[i];
            nodo.Personas[i] = 0;
            hijoDer.Personas[i] = nodo.Personas[grado+1+i];
            nodo.Personas[grado+1+i] = 0;
        }
        nodo.Personas[0] = nodo.Personas[grado];
        nodo.Personas[grado] = 0; //queda en nodo solo el valor que "subio"
        ...
        if (nodo.padre!=null) { // luego del split y asignar los hijos (hijoIzq, hijoDer), subir el valor al padre
            ...
            for (int i = 0; i<nodo.padre.Personas.length && subido==false; i++) {
                if (nodo.padre.Personas[i] == 0) {
                    nodo.padre.Personas[i] = nodo.Personas[0];
                    subido = true;
                    nodo.Personas[0] = 0;
                    ordenar(nodo.padre.Personas, 5);
                }
            }
            int posHijos = 0;
            ...
            for (int i =0; i<2*grado+3 && nodo.padre.nodo[i]!=null; i++) {
                if (nodo.padre.nodo[i].Personas[0] == nodo.Personas[0]) {
                    aqui = i;
                    break;
                }
            }
            ...
            papa.nodo[j] = null;
            ordenar(papa.Personas,5);
            ordenarNodos(papa);
            if (papa.Personas[2*grado]!=0) {
                split(papa);
            }
        }
    }
    ...
    public String recorrer(Nodo nodo) {
        ...
        for (int i =0; i<2*grado+1; i++) {
            ...
            arbol += "[ ";
            for (int j = 0; nodo.nodo[i]!=null && j<nodo.nodo[i].Personas.length; j++) {
                if (nodo.nodo[i].Personas[j] != 0) {
                    arbol += nodo.nodo[i].Personas[j] + ", ";
                }
            }
            arbol += " ]";
        }
